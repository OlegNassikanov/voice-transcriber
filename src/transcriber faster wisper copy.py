...
Требует: pip install PyQt5 pyaudio faster-whisper pyperclip numpy
"""
...
class VoiceTranscriber(QMainWindow):
        ...
        try:
            ...
            for i in range(device_count):
                ...
                if info['maxInputChannels'] > 0:
                    print(f"  [{i}] {info['name']} (вход: {info['maxInputChannels']} каналов)")
    ...
    def init_ui(self):
        ...
        self.cpu_radio.setChecked(True)  # По умолчанию CPU
            ...
            if device == "cuda":
                try:
                    ...
                    if not torch.cuda.is_available():
                        self.signals.status_update.emit('CUDA недоступна, переключаюсь на CPU')
                        device = "cpu"
                        compute_type = "int8"
                        self.cpu_radio.setChecked(True)
                except ImportError:
                    self.signals.status_update.emit('PyTorch не установлен, использую CPU')
                    device = "cpu"
                    compute_type = "int8"
                    self.cpu_radio.setChecked(True)
    ...
    def record_audio(self):
        ...
        stream = None
        try:
            # Подавляем ALSA warnings
            import os
            os.environ['ALSA_CARD'] = 'default'
            # Перенаправляем stderr чтобы скрыть ALSA ошибки
            import sys
            ...
            print(f"Используется устройство: {default_device['name']} (index: {device_index})")

            # Подавляем ALSA сообщения при открытии потока
            f = io.StringIO()
            ...
            while self.is_recording:
                try:
                    ...
                    if e.errno != pyaudio.paInputOverflowed:
                        print(f"IO Error во время записи: {e}")
                    continue
    ...
    def transcribe_audio(self):
        ...
        try:
            ...
            print(f"Аудио сохранено в: {temp_file_path}")

            # Транскрибация без указания языка (автоопределение)
